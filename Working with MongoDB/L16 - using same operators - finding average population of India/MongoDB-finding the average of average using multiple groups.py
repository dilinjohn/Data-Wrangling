...
def make_pipeline():
    ...
    group_cities = {"$group": {"_id": "$isPartof",
                               "avg_pop": {"$avg": "$population"}}}
    pipeline.append(group_cities)

    group_country = {"$group": {"_id": "India Regional City Population Average",
                                "avg_country_pop": {"$avg":"$avg_pop"}}}
    pipeline.append(group_country)
    return pipeline
